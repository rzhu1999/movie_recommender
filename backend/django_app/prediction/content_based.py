...
def calculate_cos_sim_matrix(metadata):
...
def get_similar_recommendations(title, n, metadata, cosine_sim, indices):
    ...
    if not (metadata["title"] == title).any():
        ...
        print(
            "Please enter a valid number between 1 and {}".format(len(metadata.index))
        )
    else:
        ...
        return (
            metadata[["id", "title", "director"]].iloc[movie_indices].to_dict("records")
        )
